...
def get_face_app():
    """Get or initialize the InsightFace FaceAnalysis app using antelopev2 (glintr100)."""
    global _face_app
    if _face_app is None:
        _face_app = FaceAnalysis(
            name="antelopev2",                         # Changed from buffalo_l
            providers=["CPUExecutionProvider"]          # Use CUDAExecutionProvider if you have GPU
        )
        _face_app.prepare(ctx_id=0, det_size=(640, 640))
        print(f"Loaded antelopev2 (glintr100) model")
    return _face_app
...
class Register(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """
        Register a new face with embeddings from multiple angles.
        Uses antelopev2 (glintr100) — 512-dim embeddings, higher accuracy.
        """
        serializer = FaceImageSerializer(data=request.data)
        if not serializer.is_valid():
            print(f"Serializer errors: {serializer.errors}")
        ...
        try:
            ...
            app = get_face_app()
            print(f"InsightFace app loaded: {app}")
            ...
            for field_name, angle_name in FaceImageSerializer.ANGLE_MAPPING.items():
                ...
                try:
                    ...
                    print(f"Selected face bbox area: {(face.bbox[2] - face.bbox[0]) * (face.bbox[3] - face.bbox[1]):.1f}")

                    # glintr100 produces 512-dim embeddings (buffalo_l was also 512, but
                    # glintr100 is trained on a larger dataset and generally more accurate)
                    embedding = face.embedding.astype(np.float32)
            ...
            # Validate required angles are present
            required = {'frontal', 'top', 'bottom'}
            processed = set(embeddings_by_angle.keys())
            print(f"\n--- Validation ---")
            print(f"Required: {required} | Processed: {processed}")
            ...
            # Average all angle embeddings into one representative vector
            print(f"\n--- Averaging Embeddings ---")
            embeddings_list = [np.array(emb, dtype=np.float32) for emb in embeddings_by_angle.values()]
            print(f"Averaging {len(embeddings_list)} embeddings of shape {embeddings_list[0].shape}")

            averaged_embedding = np.mean(embeddings_list, axis=0).astype(np.float32)

            # Normalize the averaged embedding (important for cosine similarity accuracy)
            norm = np.linalg.norm(averaged_embedding)
            if norm > 0:
                averaged_embedding = averaged_embedding / norm
            print(f"Averaged + normalized embedding shape: {averaged_embedding.shape}")
            print(f"Norm after normalization: {np.linalg.norm(averaged_embedding):.6f}")  # Should be ~1.0
            print(f"Contains NaN: {np.any(np.isnan(averaged_embedding))}")
            print(f"All zeros: {np.all(averaged_embedding == 0)}")

            # Convert to JSON-safe Python floats
            print(f"\n--- Converting to JSON-safe format ---")
            array_as_list = [float(x) for x in averaged_embedding.tolist()]
            print(f"Final array length: {len(array_as_list)}")  # Expect 512
            print(f"Final array sample (first 5): {array_as_list[:5]}")

            # Store per-angle embeddings (also normalized)
            angles_dict = {}
            for angle_name, embedding in embeddings_by_angle.items():
                ...
                angles_dict[angle_name] = [float(x) for x in emb_arr.tolist()]
                print(f"Stored {angle_name}: length={len(angles_dict[angle_name])}")
            ...
            print(f"Saved: ID={face_map_obj.id}, created={created}")
            print(f"  array length: {len(face_map_obj.array)}")
            print(f"  angles keys: {list(face_map_obj.angles.keys())}")
            ...
            return Response(
                {
                    ...
                    "name": name,
                    "angles_processed": list(embeddings_by_angle.keys()),
                    "embedding_dim": len(array_as_list),        # 512 for glintr100
                    "embedding_count": len(embeddings_by_angle),
                    "failed_angles": failed_angles if failed_angles else []
                },
                status=status.HTTP_201_CREATED if created else status.HTTP_200_OK
...
class Recognize(viewsets.ViewSet):
    """
    ViewSet for face recognition using antelopev2 (glintr100).

    POST /recognize/
    {
        "image": "<base64_encoded_image>",
        "threshold": 0.35   # optional — glintr100 is more accurate, slightly lower threshold works well
    }
    """
    ...
    def create(self, request):
        ...
        Note on threshold: glintr100 embeddings are L2-normalized, so cosine distance
        of 0.0 = identical, 1.0 = completely different. A threshold of 0.3–0.4 works
        well for glintr100 (buffalo_l typically needed 0.4–0.5).
        """
        serializer = RecognizeSerializer(data=request.data)
        if not serializer.is_valid():
            print(f"Serializer errors: {serializer.errors}")
        ...
        try:
            print("\n=== STARTING FACE RECOGNITION (antelopev2/glintr100) ===")
            app = get_face_app()
            data = serializer.validated_data
            image_base64 = data['image']
            threshold = data.get('threshold', 0.35)  # Slightly lower than buffalo default (0.4)
            ...
            # Normalize the probe embedding before comparison
            ...
            for stored_face in stored_faces:
                ...
                # Defensive: re-normalize stored embedding in case it was saved un-normalized
                ...
                distance = cosine(embedding, stored_embedding)
                print(f"    Cosine distance: {distance:.6f} (threshold: {threshold})")
            ...
            if not best_match_found:
                best_name = "Unknown"
                print(f"\nBest distance {best_distance:.6f} exceeds threshold {threshold} — returning Unknown")
